...
class BearDistColumnMatchingView(MDBoxLayout):
    def __init__(self, column_header):
        ...
        self.point_combo = AngelaCombobox("Point", self.column_header)
        self.point_combo.drop_down_button.set_item(self.column_header[0])
        self.point_combo.size_hint = (None, None)
        self.point_combo.size = (250, 70)
        self.easting_combo = AngelaCombobox("Easting", self.column_header)
        ...
        self.northing_combo = AngelaCombobox("Northing", self.column_header)
        ...
        self.backtarget_combo = AngelaCombobox("Back Target", self.column_header)
        self.backtarget_combo.drop_down_button.set_item(self.column_header[0])
        self.backtarget_combo.size_hint = (None, None)
        self.backtarget_combo.size = (250, 70)
        ...
        self.foretarget_combo = AngelaCombobox("Fore Target", self.column_header)
        self.foretarget_combo.drop_down_button.set_item(self.column_header[0])
        self.foretarget_combo.size_hint = (None, None)
        self.foretarget_combo.size = (250, 70)

        self.home_bottom_content.add_widget(self.point_combo)
        self.home_bottom_content.add_widget(self.easting_combo)
        self.home_bottom_content.add_widget(self.northing_combo)
        self.home_bottom_content.add_widget(self.backtarget_combo)
        self.home_bottom_content.add_widget(self.station_combo)
        self.home_bottom_content.add_widget(self.foretarget_combo)
    ...
    def reset_combo_drop_down_items(self):
        combos = (self.backtarget_combo, self.foretarget_combo, self.easting_combo,
                  self.northing_combo)
        for combo in combos:
            menu_items = []
...
if __name__ == "__main__":
    ...
    class MainApp(MDApp):
        ...
        def build(self):
            app = BearDistColumnMatchingView(["Column " + str(x) for x in range(10)])
