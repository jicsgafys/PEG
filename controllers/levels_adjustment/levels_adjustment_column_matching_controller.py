...
from views.levels_adjustment.adjustment_column_matching_view import AdjustmentColumnMatchingView
from controllers.levels_adjustment.levels_adjustment_calculate_controller import AdjustmentCalculateController


class AdjustmentColumnMatchingController(MDBoxLayout):
    def __init__(self, main_container_widget, previous_widget, data):
        ...
        self.ui = AdjustmentColumnMatchingView(self.data['column_header'])
    ...
    def move_to_next_widget(self):
        try:
            column_match = {"Point": str(self.ui.station_combo.drop_down_button.current_item),
                            "Elevation": str(self.ui.elevation_combo.drop_down_button.current_item),
                            "BM": str(self.ui.benchmark_combo.drop_down_button.current_item),
                            "From": str(self.ui.from_combo.drop_down_button.current_item),
                            "To": str(self.ui.to_combo.drop_down_button.current_item),
                            "Change_elev": str(self.ui.change_elevation_combo.drop_down_button.current_item),
                            "Weight": str(self.ui.weight_combo.drop_down_button.current_item)}
            adjustment_calculate_controller = \
                AdjustmentCalculateController(self.main_container_widget, self,
...
if __name__ == "__main__":
    ...
    class MainApp(MDApp):
        ...
        def build(self):
            app = AdjustmentColumnMatchingController(
